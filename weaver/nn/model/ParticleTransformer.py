...
class ParticleTransformer(nn.Module):
    ...
    def forward(self, x, v=None, mask=None, uu=None, uu_idx=None):
        ...
        #_logger.debug(f"x: {x.shape}, v: {v.shape if v is not None else None}, mask: {mask.shape if mask is not None else None}, uu: {uu.shape if uu is not None else None}, uu_idx: {uu_idx.shape if uu_idx is not None else None}")
        ...
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            ...
            #_logger.debug(f"permuted mask shape: {mask.permute(2, 0, 1).shape}, x shape: {x.shape}")
            attn_mask = None
